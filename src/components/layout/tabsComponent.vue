...
<script>
const vmPreliminar = () => import ('@/components/preliminary_form/preliminary_form')
const vmOptometrist = () => import ('@/components/optometrist_form/optometrist_form')
const vmOphthalmology = () => import ('@/components/ophthalmology_form/ophthalmology_form')
		...
		validateTabs (tab) {
			if(this.storePhysician.role){
				switch(this.storePhysician.role.toLowerCase()) {
					...
					break
					case 'institution':
						if (this.getRoute.c == 'H' && tab != 'history' && tab != 'constans') {
							return false
						} else {
