  ...
  <v-container>
    ...
    <v-card class="mx-auto elevation-0 blue-grey lighten-5">
      ...
      <v-window v-model="paso">
        ...
        <v-window-item :value="1">
          ...
          <visual_acuity class="px-2 py-2" ref="visualAcuityRef"></visual_acuity>
    ...
    >
      <v-card>
        ...
        <v-card-text class="headline">{{ $t('content.save_preliminary_msg')}}</v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn
                  ...
                  .then(result => {
                    ...
                    this.$refs.previousSurgeriesRef
                      ...
                      .then(result => {
                        ...
                        if(result.cirugias.length > 0) {
                          this.consultation.record.cirugias = result;
                          // console.log("result previousSurgeries: ", result)
                          if (this.paso > this.lastValidate)
                            this.lastValidate = 0;
                          resolve();
                        } else {
                          this.dialogError('dialogErr');
                          reject()
                        }
                      })
                      .catch(err => {
                  ...
                  .then(result => {
                    ...
                    this.$refs.tonometryRef
                      ...
                      .then(async result => {
                        ...
                        this.consultation.retinal_observations = retinalCamera.observations_photo
      ...
      return new Promise((resolve, reject) => {
        ...
        async function createObjPreliminaryAsync (body, vm) {
          ...
          if (objPreliminaryAux.objPreliminary) {
            ...
            objPreliminaryAux.objPreliminary.control.updated_at = moment().toISOString();
            objPreliminaryAux.objPreliminary.control.updated_by = vm.$store.getters.getPhysician._id;
            body.objPreliminary.control= objPreliminaryAux.objPreliminary.control
        ...
        createObjPreliminaryAsync(objAux.body, this)
          .then(result => {
            setTimeout(() => {
                  ...
                  this.updateOrSaveAntecedents()
                    .then(resultAntecedents => {
                      ...
                      this.saveStoreConsultation(result)
                        .then(result => {
                          if(this.$route.query.c != 'E' || this.$route.query.c != 'e'){
                            setTimeout(() => {
                              window.close()
                            }, 500);
                          }
              ...
              consultationServ
                ...
                .then(result => {
                  ...
                  this.updateOrSaveAntecedents()
                    ...
                    .catch(err => {
                      alert(
                        "No se pudo almacenar la informacion de los antecedentes " + err
                      );
                    });
                })
                .catch(err => {
    ...
    setConsultationData() {
      ...
      if (this.storeConsultation.objPreliminary) {
        if (this.storeConsultation.objPreliminary.data) {
          this.consultation.record = this.storeConsultation.objPreliminary.data.record
        }
