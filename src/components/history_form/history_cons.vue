  ...
  <v-container>
    <v-form autocomplete="off" ref="formHistoryRef" v-model="formHistory" lazy-validation>
      ...
      <v-stepper  v-model="e6" vertical non-linear>
        <v-stepper-step :step="9999" :edit-icon="'add'" complete v-if="storePhysician.user && storePhysician.user.role == 'Physician'" :editable=true @click="validateStepper(9999)">
              ...
              <v-card class="elevation-3">
                <v-card-text>
                  <v-container fluid grid-list-md px-0 py-0>
                    <v-layout row wrap>
                      <v-flex xs2 >
                        <v-text-field :readonly="true" :label="$t('title.number_of_expedient')" v-model="idQflow"></v-text-field>
                      </v-flex>
                      <v-flex xs4 >
                        <v-text-field :readonly="true" :label="$t('title.patient')" v-model="nameconsult" :rules=[rules.required]></v-text-field>
                      </v-flex>
                      <v-flex xs2 >
                      ...
                      <v-flex xs12>
                        <Vueditor ref="editor" :rules=[rules.required]> </Vueditor>
                      </v-flex>
                    </v-layout>
                  </v-container>
                </v-card-text>
                <v-card-actions>
                  <v-layout row wrap justify-end>
                      <v-flex xs6 >
                        <v-btn color="primary" @click="preview" :disabled="alert" :loading="alert">{{$t("title.pre_view")}}</v-btn>
                        <v-btn color="primary" @click="saveConsult" :disabled="alert" :loading="alert">{{$t("title.save")}}</v-btn>
                        <v-btn flat @click="dischargeImaging">{{$t("title.clear")}}</v-btn>
                      </v-flex>
                  </v-layout>
                </v-card-actions>
              </v-card>
        ...
        <div v-for="(history, z) in historyConstancy" :key="z">
          ...
          <v-stepper-content :step="z" complete :editable=true>
            ...
            <history_constancy_inf class="px-2 py-2" :myProp="history.Constancy" ref="history_constancy_inf_ref" :id="'constan'+z"></history_constancy_inf>
          </v-stepper-content>
        </div>
      </v-stepper>
    </v-form>
    ...
    <v-dialog 
      ...
      class="modalPreview"
    >
      <history_constancy_inf_preview class="px-2 py-2" :propdataPreview="dataPreview" ref="history_constancy_inf_preview" v-if="dialogPreview"></history_constancy_inf_preview>
    </v-dialog>
  </v-container>
...
const history_constancy_inf = () => import ('@/components/history_form/history_constancy_inf');
const history_constancy_inf_preview = () => import ('@/components/history_form/history_constancy_inf_preview');
...
export default {
  ...
  methods: {
    ...
    listConstancy(){
      ...
      constancyServ
      .getListConstancy(objAux)
      .then ( result => { 

          for (let i in result) {
            this.historyConstancy.push({
              "name": result[i].person.forename+' '+result[i].person.surname,
              "date": moment(result[i].date, 'YYYY-MM-DD').locale(this.$i18n.locale).format("L"),
              "file": null,
              "id": result[i]._id,
              "Constancy": result[i]
            })
    ...
    Print(e) {
      let ua = navigator.userAgent
        ...
        setTimeout(() => {
            html2canvas(document.getElementById("constan"+e),{ allowTaint: true}).then(canvas => {
       ...
       /*let ua = navigator.userAgent
        ...
        getImage('.infoLogo')
          .then(image => {
            ...
            doc.fromHTML(
            constancia.querySelector(".infoPatient"),
            15,
            5,
            {
              'width': 180
            ...
            doc.fromHTML(
            constancia.querySelector(".infConstancy"),
            15,
            30,
            {
              'width': 180
  ...
  created(){
    this.nameconsult = this.storePatient.forename+' '+this.storePatient.surname
    this.agepatient = moment().diff(this.storePatient.birthdate, "years", false)
