  ...
  <v-container>
    <v-layout row>
      <v-flex xs12 class="pb-2 text-history-wrap">
        <v-card class="totalConstancy">
          <v-card-text pb-0 mb-0 >
            <p class="text-xs-right pt-3 pr-3 pb-0 mb-0">
                <img src="../../assets/img/logoFudem.jpg" class="infoLogo" aspect-ratio="1" width="230">  
            </p>  
          </v-card-text>
          <v-card-text class="infoPatient">
            <v-container grid-list-md text-xs-left pt-0 mt-0 pb-0 mb-0>
              <v-layout row wrap>
                <v-flex xs12 pb-0 mb-0 pt-0 mt-0>
                  ...
                  >{{ $t('title.date') }}:&nbsp;</span>
                  <span class="body-1">{{date(myProp.date)}}</span>
                </v-flex>
                <v-flex xs12 pb-0 mb-0 pt-0 mt-0>
                  <span
                    class="body-1 font-weight-light font-italic"
                  >Nombre del Paciente:&nbsp;</span>
                  <span class="body-1">{{myProp.person.forename}} {{myProp.person.surname}}</span>
                </v-flex>
          ...
          <v-card-text class="text-left pt-0 mt-0 infConstancy">
            <v-container class="text-left pt-0 mt-0"> 
                <v-layout row wrap class="text-left">
                    <v-flex xs12 class="text-left" >
                        <p class="text-xs-left" v-html="textConstancy">
          ...
          <v-card-text class="digitalSignature">
            <v-container> 
                <v-layout row wrap >

                    <v-flex xs12 v-if="myProp.responsableconstancy">
                    ...
                    <v-flex offset-xs3 xs6 class="text-xs-center" v-if="myProp.responsableconstancy">
                        <v-divider light></v-divider>
                    </v-flex>
                    <v-flex xs12 class="text-xs-center" v-if="myProp.responsableconstancy">
...
export default {
  ...
  created(){
    this.textConstancy = this.myProp.description
  },
  mounted () {
    if(this.myProp && this.myProp.responsableconstancy.digital_signature){
        this.getDigital('Optometrist', this.myProp.responsableconstancy._id)
    }
       
  },
  methods: {
    dateConsultig(dateConsult){
        if(this.myProp.objPreliminary)
            return this.myProp.objPreliminary.control.created_at
        else if(this.myProp.objOphthalmology)
