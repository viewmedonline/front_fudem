  ...
  <v-container>
    <v-card style="position: fixed; max-width: 25%; width: 20%;">
      ...
      <v-card-text>
        ...
        <div>
          <span class="caption">{{$t('title.gender')}}: <span v-if="storePatient.gender">{{storePatient.gender}} / </span> <span v-if="storePatient.birthdate">{{age(storePatient.birthdate)}} {{$t('title.years')}}</span></span>
        </div>
        <div v-if="storePatient.birthdate">
          <span class="caption">{{$t('title.birthdate')}}: {{date(storePatient.birthdate)}}</span>
        </div>
        <v-list class="pt-1">
          <v-list-tile @click="fixedCompst('patient_form')">
            <v-list-tile-content>
              <v-list-tile-title>{{$t('title.information_patiente')}}</v-list-tile-title>
            </v-list-tile-content>
            <v-list-tile-action>
          ...
          <v-list-tile @click="fixedCompst('imaging')">
            <v-list-tile-content>
              <v-list-tile-title>{{$t('title.imagenology')}}</v-list-tile-title>
            </v-list-tile-content>
            <v-list-tile-action>
