<template>
  <v-container>
    <v-form autocomplete="off" ref="formTonometryRef" v-model="formTonometry" lazy-validation>
    <v-card class="elevation-3">
        <v-card-title primary-title class="blue-grey darken-1">
            <span class="subheading white--text text-capitalize">{{$t('title.tonometry')}}</span>
        </v-card-title>
    ...
    methods: {
        ...
        setTonometry () {
            if (
                ...
                if (this.storeConsultation.objPreliminary.data) this.tonometria = this.storeConsultation.objPreliminary.data.tonometria
                else if (this.storeConsultation.tonometria) this.tonometria = this.storeConsultation.tonometria
