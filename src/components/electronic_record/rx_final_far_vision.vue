<template>
  <v-container>
    <v-form autocomplete="off" ref="formRxFinalFarVisionRef" v-model="formRxFinalFarVision" lazy-validation>
    <v-card class="elevation-3">
        <v-card-title primary-title class="blue-grey darken-1">
            <span class="subheading white--text text-capitalize">{{$t('title.rx')}} {{$t('title.final')}} {{$t('title.far_vision')}}</span>
        </v-card-title>
        ...
        <v-card-text>
            <v-container fluid grid-list-md px-0 py-0>
                ...
                <v-layout row wrap>
                    ...
                    <v-flex xs2>
                        <v-text-field
                            v-model="rxFinalVisionLejano.ojoDer.esfera"
                            :rules="[]"
                    ...
                    <v-flex xs2>
                        <v-text-field
                            v-model="rxFinalVisionLejano.ojoDer.cilindro"
                            :rules="[]"
                            :readonly="validateRead()">
                        </v-text-field>
                    </v-flex>
                    <v-flex xs2>
                        <v-text-field
                            v-model="rxFinalVisionLejano.ojoDer.eje"
                            :rules="[]"
                            :readonly="validateRead()">
                        </v-text-field>
                    </v-flex>
                    <v-flex xs2>
                        <v-text-field
                            v-model="rxFinalVisionLejano.ojoDer.prisma"
                            :rules="[]"
                            :readonly="validateRead()">
                        </v-text-field>
                    </v-flex>
                    <v-flex xs2>
                        <v-text-field
                            v-model="rxFinalVisionLejano.ojoDer.av"
                            :rules="[]"
                            :readonly="validateRead()">
                        </v-text-field>
                    </v-flex>
                ...
                <v-layout row wrap>
                    ...
                    <v-flex xs2>
                        <v-text-field
                            v-model="rxFinalVisionLejano.ojoIzq.esfera"
                            :rules="[]"
                            :readonly="validateRead()">
                        </v-text-field>
                    </v-flex>
                    <v-flex xs2>
                        <v-text-field
                            v-model="rxFinalVisionLejano.ojoIzq.cilindro"
                            :rules="[]"
                            :readonly="validateRead()">
                        </v-text-field>
                    </v-flex>
                    <v-flex xs2>
                        <v-text-field
                            v-model="rxFinalVisionLejano.ojoIzq.eje"
                            :rules="[]"
                            :readonly="validateRead()">
                        </v-text-field>
                    </v-flex>
                    <v-flex xs2>
                        <v-text-field
                            v-model="rxFinalVisionLejano.ojoIzq.prisma"
                            :rules="[]"
                            :readonly="validateRead()">
                        </v-text-field>
                    </v-flex>
                    <v-flex xs2>
                        <v-text-field
                            v-model="rxFinalVisionLejano.ojoIzq.av"
                            :rules="[]"
                            :readonly="validateRead()">
                        </v-text-field>
                    </v-flex>
        ...
        saveRxFinalFarVision () {
            return new Promise ((resolve, reject) => {
                if (this.$refs.formRxFinalFarVisionRef.validate()) {
                    resolve(this.rxFinalVisionLejano)
                }else{
                    reject(false)
                }
            })
        },
        setRxFinalFarVisionRef(){
