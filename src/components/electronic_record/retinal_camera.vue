  ...
  <v-container>
    ...
    >
          ...
          <v-container fluid grid-list-md px-0 py-0>
            <v-layout row wrap>
              <v-flex xs2>
                ...
                <v-radio-group
                  v-model="photo_retinal"
                  row
                  :rules="preliminary ? [rules.required] : []"
                  :disabled="optometrist"
                >
      ...
      <v-card
        ...
        v-if="photo_retinal == 'Si' && optometrist"
      >
        <v-card-title primary-title class="blue-grey darken-1">
          <span class="subheading white--text text-capitalize"
            >Hallazgo en fotografía</span
          >
        </v-card-title>
        <v-divider light class="vm-border-color-2"></v-divider>
        <v-card-text>
            ...
            <v-layout row wrap>
              <v-flex xs12>
                <!-- <v-select :items="items" label="Seleccione" :rules="[rules.required]"
                                    v-model="photo_retinal"></v-select> -->
                <v-radio-group
                  v-model="findings_photo"
                  row
                  :rules="optometrist ? [rules.required] : []"
                >
              ...
              <v-flex xs12>
                <v-textarea
                  label="Observaciones en fotografía"
                  v-model="observations_photo"
                  :disabled="!findings_photo"
                ></v-textarea>
              </v-flex>
            </v-layout>
          </v-container>
  ...
  methods: {
    ...
    setDataRetinalCamera() {
      if (this.storeConsultation.objPreliminary) {
        ...
        this.observations_photo =
          this.storeConsultation.objPreliminary.data.retinal_observations;
        this.retinal_notes =
          this.storeConsultation.objPreliminary.data.retinal_notes;
      }
    },
  },
  computed: {
