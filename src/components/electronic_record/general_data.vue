        ...
        <v-form autocomplete="off" ref="formGeneralDataRef" v-model="formGeneralData" lazy-validation>
            <v-card class="elevation-3">
                <v-card-title primary-title class="blue-grey darken-1">
                    <span class="subheading white--text text-capitalize">{{ $t('title.general_data') }}</span>
                </v-card-title>
                ...
                <v-card-text>
                    <v-container fluid grid-list-md px-0 py-0>
                        <v-layout row wrap>
                            <v-flex xs12 sm4>
                                <v-text-field v-model="expedient" :label="$t('title.number_of_expedient')"
                                    :rules="validateRequired" readonly>
                                </v-text-field>
                            </v-flex>
                            <v-flex xs12 sm8>
                                <v-text-field v-model="name" :label="$t('title.name')" :rules="validateRequired"
                                    readonly>
                                </v-text-field>
                            </v-flex>
                        </v-layout>
                        <v-layout row wrap>
                            <v-flex xs6 sm3>
                                <v-text-field v-model="gender" :label="$t('title.gender')" :rules="validateRequired"
                                    readonly>
                                </v-text-field>
                            </v-flex>
                            <v-flex xs6 sm3>
                            ...
                            <v-flex xs6 sm3>
                                <v-switch :label="`${$t('title.read_and_write')}`" v-model="readAndWrite"
                                    readonly></v-switch>
                            </v-flex>
                            <v-flex xs6 sm3>
                        ...
                        <v-layout row wrap>
                            <v-flex xs12>
                                <v-textarea name="input-7-1" :label="$t('title.reasonforconsultation')"
                                    :rules="validateRequired" outline v-model="reasonConsultation"
                                    :readonly="validateRead()"></v-textarea>
                            </v-flex>
                        </v-layout>
                        <v-layout row wrap>
                            <v-flex xs12 sm4>
                                <v-select v-model="typeLenses" :items="lenses" :label="$t('title.type_of_lenses')"
                            ...
                            <v-flex xs12 sm4 v-else>
                                <v-select v-model="typeConsultation" :items="Consultations"
                                    :label="$t('title.type_of_consultation')" :rules="validateRequired"
    ...
    methods: {
            ...
            return new Promise((resolve, reject) => {
                if (this.$refs.formGeneralDataRef.validate()) {
                    let valor = {
                        ...
                        typeConsultation: this.$route.query.c == 'E' ? this.$route.query.c : this.typeConsultation
                    }
                    resolve(valor)
                } else {
                    reject(false)
        ...
        setPersonData() {
            ...
            if (this.$route.query.c == 'E')
                ...
                this.typeConsultation = this.storeConsultation.generalData ? this.storeConsultation.typeConsultation : ''

            // Validando si existe el ojbPreliminary y luego si existe el data del mismo
            if (this.storeConsultation.objPreliminary) {
                if (this.storeConsultation.objPreliminary.data) {
                    this.typeLenses = this.storeConsultation.objPreliminary.data.generalData.typeLense || null
                    this.reasonConsultation = this.storeConsultation.objPreliminary.data.reasonConsultation || null
                } else {
