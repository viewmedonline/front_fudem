<template>
  <v-container>
    <v-form
      autocomplete="off"
      ref="formVisualAcuityOptRef"
      v-model="formVisualAcuityOpt"
      lazy-validation
    >
            ...
            <v-layout row wrap>
              <v-flex offset-xs3 xs6>
                <v-textarea
                  outline
                  name="input-7-1"
                  v-model="agudezaVisualOPT.observation"
    ...
    <v-dialog v-model="historyVisual" scrollable max-width="40%">
      <v-card>
        ...
        <v-divider></v-divider>
        <v-card-actions>
          <v-btn color="blue darken-1" flat @click="historyVisual = false">{{
            $t("title.close")
          }}</v-btn>
        </v-card-actions>
      </v-card>
      ...
      if (result.length > 0) {
        ...
        result.forEach((element) => {
          if (element.objOptometrist && element.objOptometrist.data)
            this.visualAcuity.push(
              element.objOptometrist.data.agudezaVisualOPT
            );
          else this.visualDisabledButton = true;
        });
      } else {
  ...
  methods: {
    ...
    setVisualAcuity() {
      if (
        ...
        this.agudezaVisualOPT =
          this.storeConsultation.agudezaVisualOPT ||
          this.storeConsultation.agudezaVisual;
      }

      if (this.storeConsultation.tonometria) {
        this.agudezaVisualOPT.ojoDer.autoTonometria =
          this.storeConsultation.tonometria.ojoDer || null;
        this.agudezaVisualOPT.ojoIzq.autoTonometria =
          this.storeConsultation.tonometria.ojoIzq || null;
      }
    },
  },
  mounted() {
